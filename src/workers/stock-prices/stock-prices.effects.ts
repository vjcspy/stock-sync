...
const checkStockPriceStatus$ = createEffect(action$ => action$.pipe(
...
const getStockPrice$ = createEffect(action$ => action$.pipe(
  ofType(getStockPricesAction),
  concatMap((action) => from(getPrice(action.payload.code, action.payload.year)).pipe(
    map((priceData) => {
      if (priceData === null) {
...
const whenGotStockPrices$ = createEffect((action$, state$) => action$.pipe(
  ...
  concatMap((d) => {
    ...
    if (action.payload.data.totalElements === 0) {
      ...
      if (moment().year() > successYear) {
        return of(getStockPricesAction({
          code: action.payload.code,
      ...
      if (!Array.isArray(saveData.data) || saveData.data.length === 0) {
        return of(saveStockPriceErrorAction({
      ...
      if (action.payload.year === moment().year()) {
        ...
        saveData['data'] = saveData.data.filter((p: any) => lastDate.isBefore(moment(p['date'], 'YYYY-MM-DD')));
...
const repeat$ = createEffect(action$ => action$.pipe(
...
const exit$ = createEffect(action$ => action$.pipe(
...
export const stockPricesEffects = [checkStockPriceStatus$, getStockPrice$, whenGotStockPrices$, repeat$, exit$];
