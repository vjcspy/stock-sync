...
const syncPrice = async (code: string) => {
  ...
  const getPriceAndSave = async (year: number) => {
    ...
    console.log(`[${code}] year ${year} with total elements: ${priceData?.totalElements}`);
    if (priceData === null) {
      ...
      if (priceData.totalElements === 0) {
        ...
        if (year === moment().year()) {
          ...
          if (_savePrices.length === 0) {
            ...
            await savePrices(code, {...priceData,data:_savePrices});

            console.log(`[${code}] saved ${_savePrices.length} records and finish`);
            return true;
          }
        } else {
          await savePrices(code, priceData);
          console.log(`[${code}] saved ${priceData.length} records and query next year`);
          return getPriceAndSave(year + 1);
        }
      }
    }
